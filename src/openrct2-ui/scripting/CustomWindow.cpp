...
namespace OpenRCT2::Ui::Windows
{
    ...
    struct CustomWindowDesc
    {
        ...
        static CustomWindowDesc FromJSValue(JSContext* ctx, JSValue desc)
        {
            ...
            result.size.width = GetInt(ctx, desc, "width");
            result.size.height = GetInt(ctx, desc, "height");
        ...
        void OnPrepareDraw() override
        {
            ...
            for (const auto& widget : widgets)
            {
                if (widget.type == WidgetType::scroll)
                {
                    ...
                    if (listView.GetScrollbars() == ScrollbarType::Vertical || listView.GetScrollbars() ==
                    ScrollbarType::Both)
                    {
