    ...
    private:
        ...
        static JSValue image(JSContext* ctx, JSValue thisVal, int argc, JSValue* argv)
        {
            ...
            else
            {
                if (data->_colour)
                {
                    img = img.WithPrimary(static_cast<Colour>(*data->_colour));
                }
                if (data->_secondaryColour)
                {
                    img = img.WithSecondary(static_cast<Colour>(*data->_secondaryColour));
                }
            }

            GfxDrawSprite(data->_rt, img.WithTertiary(static_cast<Colour>(data->_tertiaryColour.value_or(0))), { x, y });
            return JS_UNDEFINED;
        }
        ...
        static JSValue text(JSContext* ctx, JSValue thisVal, int argc, JSValue* argv)
        {
            ...
            JS_UNPACK_INT32(y, ctx, argv[2]);
            GraphicsData* data = gScGraphicsContext.GetOpaque<GraphicsData*>(thisVal);
            DrawText(data->_rt, { x, y }, { static_cast<Colour>(data->_colour.value_or(0)) }, text.c_str());
            return JS_UNDEFINED;
        }
    };
} // namespace OpenRCT2::Scripting
